<script lang="ts">
  ...
  const BADGE_MAP: Record<string, { icon: Component; label: string; classes: string }> = {
    filesystem: { 
      icon: Folder, 
      label: "FS", 
    ...
    database: { 
      icon: Database, 
      label: "DB", 
      classes: "bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 border-purple-200 dark:border-purple-800" 
    },
    read_only: { 
      icon: Eye, 
      label: "Read", 
  ...
  let info = $derived(BADGE_MAP[capability] ?? { 
    icon: HelpCircle, 
    label: capability, 
